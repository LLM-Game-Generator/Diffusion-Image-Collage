...
def redistribute_vertices(geom, gap):
    if geom.geom_type == 'LineString':
        ...
        from shapely.geometry import MultiLineString
        lines = [redistribute_vertices(g, gap) for g in geom.geoms if g.geom_type in ['LineString', 'LinearRing', 'MultiLineString']]
        return MultiLineString([l for l in lines if not l.is_empty])
    else:
        return geom
...
class InteriorAngle:
    def __init__(self, start_vector, end_vector):
        self.root = Node(start_vector, end_vector)
        self.tolerance = math.pi / 12.0
        self.cut_list = []

    def add_cut(self, cut_vector):
        u_vector = create_unit_vector(cut_vector)
        # 1. 檢查是否已經存在極其相近的切割線，避免樹無限增長
        for past_v in self.cut_list:
            if np.allclose(u_vector, past_v, atol=1e-4):
                return

        # 2. 只有在區間內的才加入
        if self.root.contains(u_vector):
            self.cut_list.append(u_vector)
            self._add_cut(u_vector, self.root)
        else:
            # 這裡不再直接噴報錯，而是微調向量嘗試加入，或優雅跳過
            # 在動態影格中，這能確保批次處理不中斷
            pass
    ...
    def _add_cut(self, cut_vector, cur_node):
        if cur_node.is_leave():
            ...
            elif cur_node.right_child.contains(cut_vector):
                self._add_cut(cut_vector, cur_node.right_child)
            else:
                # 若因浮點誤差兩邊都不進，強制送入夾角較小的一邊
                dist_l = angle_between_vectors(cur_node.left_child.end_vector, cut_vector)
                dist_r = angle_between_vectors(cut_vector, cur_node.right_child.start_vector)
                if dist_l < dist_r:
                    self._add_cut(cut_vector, cur_node.left_child)
                else:
                    self._add_cut(cut_vector, cur_node.right_child)
    ...
    def list_leaves(self):
        leaves = []
        self._list_leaves(self.root, leaves)
        return leaves

    def _list_leaves(self, cur_node, leaves):
        if cur_node.is_leave():
            leaves.append(cur_node)
        else:
            self._list_leaves(cur_node.left_child, leaves)
            self._list_leaves(cur_node.right_child, leaves)
