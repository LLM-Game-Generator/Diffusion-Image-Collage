...
def plot_medial_axis(contour, medial_axis, distance):
    ...
    main_fig, main_ax = plt.subplots(nrows=1, ncols=1, num='Layout',
                                     figsize=(medial_axis.shape[1] / 100, medial_axis.shape[0] / 100))
    plt.subplots_adjust(left=0, bottom=0, right=1, top=1)
...
def find_projection_pair(boundary_vertices, x, y, projection_distance, interior=True):
    ...
    for neighbor in neighbors:
        vector1 = np.array(list(boundary_vertices.coords)[(neighbor + 1) % length]) - np.array(
            list(boundary_vertices.coords)[neighbor])
        vector2 = np.array(list(boundary_vertices.coords)[neighbor]) - np.array([x, y])
        if abs(vector_angle(vector1,
                            vector2) - 0.5 * math.pi) < math.pi / 18:  # close to 90 degrees with 10 degrees of error
            possible_projections.append(neighbor)
        elif abs(vector_angle(vector1,
                              vector2) - 0.5 * math.pi) < math.pi / 9:  # close to 90 degrees with 20 degrees of error
...
def plot_projection_pair(polygon, G, boundary_vertices, width, height):
    ...
    for v in G.nodes:
        ...
        for projection in projections:
            ...
            plt.plot([medial_vertex_x, projection_x], [medial_vertex_y, projection_y], marker='o', markersize=1,
                     linewidth=1)
...
def mark_extended_corner(boundary_vertices_dict_ext, boundary_vertices_dict_int, boundary_vertices, G_ext,
                         end_vertices_ext, corner_mapping_ext):
    ...
    for end in end_vertices_ext:
        ...
        for edges in nx.bfs_edges(G_ext, end):
            ...
            if len(projections) == 2:
                if (G_ext.degree(edges[1]) <= 2 and ((
                        not isinstance(boundary_vertices_dict_int[coords[projections[0]]], Corner) and not isinstance(
                        boundary_vertices_dict_int[coords[projections[1]]], Corner)))):
...
def select_representative_cuts(raw_cuts, boundary_vertices_dict_ext, boundary_vertices_dict_int):
    ...
    while copied:
        ...
        distance = [cut_distance_to_center(cut2boundary_group(element, boundary_vertices_dict_ext)) for element in
                    equivalence_set]
...
def get_extented_arc_length(boundary_vertices_dict_extended, distance=5):
    ...
    for v in boundary_vertices_dict_extended:
        if isinstance(boundary_vertices_dict_extended[v], Corner) and boundary_vertices_dict_extended[
            v].index in length_dict:
            length_dict[boundary_vertices_dict_extended[v].index] = length_dict[boundary_vertices_dict_extended[
                v].index] + distance
        elif isinstance(boundary_vertices_dict_extended[v], Corner) and not (
                boundary_vertices_dict_extended[v].index in length_dict):
...
def extension_strength(cut, extended_arc_length, distance_adjusted_ext):
    if isinstance(distance_adjusted_ext[cut[0]], Corner) and isinstance(distance_adjusted_ext[cut[1]], Corner):
        es = (extended_arc_length.get(distance_adjusted_ext[cut[0]].index, 0) + extended_arc_length.get(
            distance_adjusted_ext[cut[1]].index, 0)) / 2
    ...
    for endv in corner_mappings:
        ...
        if corner_mappings[endv] in corner_endpoints:
            for projection in corner_endpoints[corner_mappings[endv]]:
                ...
                plt.plot([x, coords[projection][0]], [y, coords[projection][1]], marker='o', markersize=1, color='blue',
                         linewidth=1)
            try:
...
def corner_calibrated_endpoints(corner_mappings, corner_endpoints, boundary_vertices, G, expansion_parameter=5):
    ...
    for corner in corner_endpoints:
        ...
        if len(projections) == 1:
            calibrated[corner] = [(projections[0] - expansion_parameter) % total,
                                  (projections[0] + expansion_parameter) % total]
    return calibrated
    ...
    for endv in corner_mappings:
        ...
        if corner_mappings[endv] in corner_endpoints:
            ...
            if len(projections) == 2:
                ...
                v1 = point_tangent_vector(boundary_vertices, (projections[0] - expansion_parameter) % total,
                                          is_first=True)
                v2 = point_tangent_vector(boundary_vertices, (projections[0] + expansion_parameter) % total,
                                          is_first=False)
                angle = interior_angle(v1, v2)
                tt = "{a:.2f}".format(a=angle * 180 / math.pi)
                plt.text(x, y, tt, fontsize=12)
...
class Node:
    ...
    def __repr__(self):
        return 'Node(' + str(self.start_vector) + ", " + str(self.end_vector) + ", angle=" + str(
...
def generate_cuts(filename, output_path):
    ...
    if ma_ext[0].sum() > 0:
        ...
        ec = mark_extended_corner(component_adjusted_ext, component_adjusted_int, boundary_vertices, G_ext, endv_ext,
                                  corner_mapping_ext)
        ...
        protrusion_strength_filter = np.array(
            [protrusion_strength(r[0], r[1], boundary_vertices) > protrusion_threshold for r in
             denoised])  # priority decreased
        extension_strength_filter = np.array(
            [extension_strength(r, eal, component_adjusted_ext) < extension_strength_threshold for r in
             denoised])  # priority decreased
        ...
        for corner in corner_ordered:
            ...
            if corner in corner_2_cuts:
                ...
                while cuts:
                    ...
                    difference = [interior_angle(cut_vector, past_cut_angle) for past_cut_angle in
                                  concave_corner.cut_list]
                    if all(d > angle_equivalence_threshold for d in difference):
                        concave_corner.add_cut(cut_vector)
                        if current_cut not in final_cuts:
                            final_cuts.append(current_cut)
                        # Check convexity
                        angles = [leaf.angle() for leaf in
                                  concave_corner.list_leaves()]  # angles_between(acc, start_vector, end_vector)
